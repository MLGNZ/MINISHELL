...
/*   Updated: 2025/08/27 15:38:51 by mlagniez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int update_lst(t_list **p_lst_a, t_list **p_lst_b) //change donc proteger ses appels
{
	t_list *env;
	t_list *temp;
	int cat;
...
t_list *exists_in_vars(char *content, t_list *vars, int *cat)
{
	...
	*cat = 0;
	...
	while (vars)
	{
		if (!ft_strncmp(vars->content, content, sok) && (!((char *)vars->content)[sok] || ((char *)vars->content)[sok] == '=' || ((char *)vars->content)[sok] == '+'))
		{
