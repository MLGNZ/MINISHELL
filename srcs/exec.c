...
/*   Updated: 2025/09/04 18:13:16 by tchevall         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_pipe(t_pl *pl)
{
	if ((pl->position == LAST && pl->has_red_in))
		return ;
	// if (pl->position != LAST && pl->has_red_out)
	// 	return ;
...
int	handle_built_in(t_pl **pls, int *i, t_ms *ms)
{
	...
	else
	{
		...
		{
			if (!ft_strncmp(pls[*i]->cmd, "echo", 5) && !pls[*i]->cmd_args[1])
			{
				dup2(pls[*i]->fd_in, 1);
				dup2(pls[*i]->fd_out, 0);
			}
...
int	handle_execve(t_pl **pls, int i, t_ms *ms)
{
	...
	handle_pipe(pls[i]);
	if (pls[i]->redir && !pls[i]->has_red_out)
	{
		close(pls[i]->current_pipe[1]);
		dup2(pls[i]->current_pipe[0], 0);
		close(pls[i]->current_pipe[0]);
